...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        help=(
            "File suffix to include when scanning the repository. Repeat for multiple "
            "extensions (defaults to a sensible built-in list)."
        ),
    )
    return parser
...
def main() -> None:
    ...
    comparison = compare_agents(
        ...
        top_k=args.top_k,
        include_extensions=tuple(args.extensions) if args.extensions else None,
    )
